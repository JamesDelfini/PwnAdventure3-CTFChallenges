...
# Note: Trait the alphanumeric as hexadecimal and the max value is 1F.
...
def KeyVerifyCall2(key8, key2Constants, key15Cntrl, key2):
    ...
    print("KEY8 - INITIAL {0}".format(key8)
          ) if debugPart4 else 0
    print("KEY12 - INITIAL [{1}] {0}".format(key12, hex(id(key2)))
          ) if debugPart4 else 0
    print() if debugPart4 else 0
    ...
    for i in range(96):
        ...
        # print("KEY12 - {1} {0}".format(key12, 'IN' if key8[i >> 3] & rolVal != 0 else 'OUT')
        #       ) if debugPart4 else 0
...
def KeyVerify(input):
    if len(input) == 25:
        if input.find(' ') != -1 or input.find('-') != -1:
            ...
            print(f'Input - {input}') if debugPart1 else 0
            print(f'Input KEY25 - {key1}') if debugPart1 else 0
            print() if debugPart1 else 0
            # Copy the first 16 of the data of @key1
            key1_tmp = resize(key1, 16)
            print(f'Input KEY16 - {key1_tmp} - Original') if debugPart1 else 0
            key1_tmp = ASM_MMXRight(key1_tmp, 8)
            print(f'Input KEY16 - {key1_tmp} >>> 8') if debugPart1 else 0
            key1_tmp = ASM_MMXRight(key1_tmp, 4)
            print(f'Input KEY16 - {key1_tmp} >>> 4') if debugPart1 else 0
            key1_tmp = ASM_MMXRight(key1_tmp, 2)
            print(f'Input KEY16 - {key1_tmp} >>> 2') if debugPart1 else 0
            key1_tmp = ASM_MMXRight(key1_tmp, 1)
            print(f'Input KEY16 - {key1_tmp} >>> 1') if debugPart1 else 0

            convertLastLetter = (ASM_BitRight(
                key1_tmp[15] + key1Verify1)) & HexToInt('1F')
            print(
                f'Last Letter: {ASM_BitRight(key1[24])}, Decoded: {convertLastLetter} | {ASM_BitRight(key1_tmp[15])} | {key1Verify1}') if debugPart1 else 0
            print() if debugPart1 else 0
            ...
            if (ASM_BitRight(key1[24]) == (ASM_BitRight(key1_tmp[15] + key1Verify1)) & HexToInt('1F')):
                ...
                for i in range(len(key1)):
                    if i+1 <= 24:
                        print("C1: {0:02}, C2: {1:02}".format(
                            charCnt1, charCnt2)) if debugPart2 else 0

                        if ((key1[i] & 1) != 0):
                            print("KEY24[{0:02}]: {1:02X} & 1 = {2}".format(
                                i, key1[i], key1[i] & 1 != 0)) if debugPart2 else 0

                            tmpI = charCnt1 - 2
                            print("    {0:08b} [{0:02d}]".format(
                                tmpI)) if debugPart2 else 0
                            tmpI >>= 3
                            print(">>3 {0:08b} [{0:02d}]".format(
                                tmpI)) if debugPart2 else 0

                            print() if debugPart2 else 0

                            tmpJ = charCnt2 - 2
                            print("    {0:08b} [{0:02d}]".format(
                                tmpJ)) if debugPart2 else 0
                            tmpJ &= 7
                            print("&   {0:08b} [{0:02d}]".format(
                                7)) if debugPart2 else 0
                            print(
                                "->  {0:08b} [{0:02d}]".format(tmpJ)) if debugPart2 else 0

                            print() if debugPart2 else 0

                            tmpK = ASM_ClearBitRight(tmpI) + 1
                            print("    {0:08b} [{0:02d}]".format(
                                tmpK)) if debugPart2 else 0
                            tmpK <<= ASM_BitRight(tmpJ)
                            print("<<{1} {0:08b} [{0:02d}]".format(
                                tmpK, tmpJ)) if debugPart2 else 0

                            print() if debugPart2 else 0

                            tmpL = ASM_BitRight(tmpI)
                            print("    {0:08b} [{0:02d}]".format(
                                key2[tmpL])) if debugPart2 else 0
                            print("|   {0:08b} [{0:02d}]".format(
                                tmpK)) if debugPart2 else 0
                            key2[tmpL] = ASM_BitRight(key2[tmpL]) | tmpK
                            print(
                                "->  {0:08b} [{0:02d}]".format(ASM_BitRight(key2[tmpL]) | tmpK)) if debugPart2 else 0
                            print(key2) if debugPart2 else 0
                            print() if debugPart2 else 0

                        if ((key1[i] & 2) != 0):
                            print("KEY24[{0:02}]: {1:02X} & 2 = {2}".format(
                                i, key1[i], key1[i] & 2 != 0)) if debugPart2 else 0
                        ...
                        if ((key1[i] & 4) != 0):
                            print("KEY24[{0:02}]: {1:02X} & 4 = {2}".format(
                                i, key1[i], key1[i] & 4 != 0)) if debugPart2 else 0
                        ...
                        if ((key1[i] & 8) != 0):
                            print("KEY24[{0:02}]: {1:02X} & 8 = {2}".format(
                                i, key1[i], key1[i] & 8 != 0)) if debugPart2 else 0
                        ...
                        if ((key1[i] & 16) != 0):
                            print("KEY24[{0:02}]: {1:02X} & 16 = {2}".format(
                                i, key1[i], key1[i] & 16 != 0)) if debugPart2 else 0
                ...
                print("KEY24 - {0}".format(key2)) if debugPart2 else 0
                ...
                print("KEY15 - INITIAL {0}".format(key15)
                      ) if debugPart3 else 0
                print("KEY8 - INITIAL {0}".format(key8)
                      ) if debugPart3 else 0
                # print() if debugPart3 else 0
                ...
                for i in range(24):
                    ...
                    if (key2LoopCntrl[i >> 3] & rolVal != 0):
                        ...
                        print("KEY15 - {0}".format(key15)) if debugPart3 else 0
                ...
                if testing:
                    ...
                    if input == 'ABCDEFHJKLMNPQRTZZZZZZZZH':
                        testKey15a = ['76', 'E2', '6A', '82', '30',
                                      '85', '8D', '85', 'B0', '4D', 'A3', '00']
                        testKey15b = ['21', '3F', '52', '14', '50',
                                      '57', '4E', '41', '44', '56', '33', '00']
                    elif input == 'ZZZZZZZZZZZZZZZZZZZZZZZZ8':
                        testKey15a = ['C4', 'AB', '6F', '37', '08',
                                      '6D', '7C', '7F', '53', '8B', '17', '03']
                        testKey15b = ['21', '3F', '52', '14', '50',
                                      '57', '4E', '41', '44', '56', '33', '00']
                    elif input == 'AAAAAAAAAAAAAAAAYYYYYYYYH':
                        testKey15a = ['1C', '4F', '97', '78', '55',
                                      'B5', '93', '2A', 'D5', '3A', '0E', '00']
                        testKey15b = ['00', 'BB', '42', '16', '50',
                                      '57', '4E', '41', '44', '56', '33', '00']
                    elif input == 'AAAAAAAAAAAAAAAAZZZZZZZZR':
                        testKey15a = ['7F', 'B6', '0E', '78', 'EE',
                                      '03', 'Df', '27', 'DC', 'DA', '64', '01']
                        testKey15b = ['21', '3F', '52', '14', '50',
                                      '57', '4E', '41', '44', '56', '33', '00']
                    else:
                        valid = False
                    ...
                    if valid:
                        ...
                        for i in range(12):
                            if key15[i] == HexToInt(testKey15a[i]):
                                testValid1 = True
                            else:
                                testValid1 = False
                                break
                        ...
                        for i in range(12):
                            if key15b[i] == HexToInt(testKey15b[i]):
                                testValid2 = True
                            else:
                                testValid2 = False
                                break
                        ...
                        print(
                            'Test 1 - Passed') if testValid1 else print('Test 1 - Failed')
                        print(
                            'Test 2 - Passed') if testValid2 else print('Test 2 - Failed')
                    else:
...
keyInput = 'H5P2PKPKWQD3ZMA1AZMPFT13X'
KeyVerify(keyInput)
